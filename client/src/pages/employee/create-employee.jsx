...
function CreateEmployee() {
    ...
    const addEmployee = (event) => {
        ...
        setIsLoading(true)
        axios.post(`${config.apiBaseurl}/employee`, {
    ...
    const getGenders = () => {
        axios.get(`${config.apiBaseurl}/gender`).then((response) => {
            let genders = []
            for (let gender of response.data) {
                genders.push({ value: gender.Name, text: gender.Name })
            }
            setGenders(genders)
        }, (error) => {
            console.log(error)
        ...
        <div className='w-full h-screen px-10'>
            ...
            {!isLoading &&
                <form onSubmit={addEmployee}>
                    <h1 className='text-left uppercase  text-xl font-medium py-5 text-gray-700'>Add Employee</h1>
                    <div className='mb-6 grid grid-cols-1 sm:grid-cols-1 md:grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-7 text-white'>
                        ...
                        <Input value={employee.MotherName}
                            ...
                            label={'Mother Name'}
                            required={true} />
                        <Input type={"email"} value={employee.EmailId}
                            onChange={event => setEmployee({ ...employee, EmailId: event.target.value })}
                            id={'EmailId'}
                            label={'Email Id'}
                            required={true} />
                        <Input type={"number"} value={employee.MobileNumber}
                        ...
                        <Input value={employee.Education}
                            ...
                            label={'Education'}
                            required={true} />
                        <Select id="Gender" selectedValue={employee?.Gender}
                            onValueChange={(event) => setEmployee({ ...employee, Gender: event.target.value })}
                            options={genders} label="Select Gender" />
                        <Select id="MaritalStatus" selectedValue={employee?.MaritalStatus}
                        ...
                        <Select id="BloodGroup" selectedValue={employee?.BloodGroup}
