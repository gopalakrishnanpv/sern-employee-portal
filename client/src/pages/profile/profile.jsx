...
function Profile() {
    ...
    useEffect(() => {
        ...
        axios.get(`${config.apiBaseurl}/employee`).then((response) => {
            axios.get(`${config.apiBaseurl}/auth`).then((user) => {
                const employee = response.data.filter(x => x.UserName === `ap\\${user.data.username}`)[0]
    ...
    return (
        <div className='w-full h-screen px-5'>
            ...
            {(!isLoading && employee) &&
                <div>
                    <h1 className='text-left uppercase  text-md font-medium py-5 text-gray-700'>My Profile</h1>
    ...
    function detailEntry(labelName, labelValue) {
        return (
            <div className="grid grid-cols-6 w-full p-2">
                <p className="text-sm font-semibold col-span-2">{labelName}: </p>
                <p className="text-sm font-normal col-span-4">{labelValue}</p>
            </div>
        )
    }
